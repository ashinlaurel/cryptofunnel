import React from "react";
import tw from "twin.macro";
import { css } from "styled-components/macro"; //eslint-disable-line
import AnimationRevealPage from "../helpers/AnimationRevealPage.js";
import Hero from "../components/hero/TwoColumnWithInput.js";
import Features from "../components/features/ThreeColWithSideImage.js";
import MainFeature from "../components/features/TwoColWithButton.js";
import MainFeature2 from "../components/features/TwoColWithTwoHorizontalFeaturesAndButton.js";
import FeatureWithSteps from "../components/features/TwoColWithSteps.js";
import Pricing from "../components/pricing/ThreePlans.js";
import Testimonial from "../components/testimonials/TwoColumnWithImageAndRating.js";
import FAQ from "../components/faqs/SingleCol.js";
import GetStarted from "../components/cta/GetStarted";
import Footer from "../components/footers/MiniCenteredFooter";
// import heroScreenshotImageSrc from "../images/hero-screenshot-1.png";
import macHeroScreenshotImageSrc from "../images/hero-screenshot-2.png";
import stepsPic from "../images/stepsPic.png";
import prototypeIllustrationImageSrc from "../images/prototype-illustration.svg";
import valuespic from "../images/valuespic.png";
import { ReactComponent as BriefcaseIcon } from "feather-icons/dist/icons/briefcase.svg";
import { ReactComponent as MoneyIcon } from "feather-icons/dist/icons/dollar-sign.svg";
import UserProfile from "../helper/auth/UserProfile.js";
import Header from "../components/headers/light";

export default () => {
  const Subheading = tw.span`uppercase tracking-widest font-bold text-gray-100`;
  const HighlightedText = tw.span`text-green-300 `;
  console.log(UserProfile.getRole());

  return (
    <AnimationRevealPage>
      <Header roundedHeaderButton={true} />
      <div className=" text-white mx-10 md:mx-32 lg:mx-64 my-48">
        <>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              pageBreakInside: "avoid",
              orphans: 2,
              widows: 2,
              pageBreakAfter: "avoid",
            }}
            align="left"
          >
            <span style={{ fontSize: "xx-large" }}>PRIVACY POLICY</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>The </span>
            <span style={{ fontSize: "medium" }}>
              <strong>cfsquad </strong>
            </span>
            <span style={{ fontSize: "medium" }}>
              respects your privacy and is committed to protecting your personal
              data. This privacy notice will inform you as to how we look after
              your personal data when you visit our website and application
              (regardless of where you visit it from) and tell you about your
              privacy rights and how the law protects you.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              This privacy notice is provided in a layered format so you can
              click through to the specific areas set out below. Please also use
              the Glossary to understand the meaning of some of the terms used
              in this privacy notice.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              1. IMPORTANT INFORMATION AND WHO WE ARE
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              2. THE DATA WE COLLECT ABOUT YOU
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              3. HOW IS YOUR PERSONAL DATA COLLECTED
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              4. HOW WE USE YOUR PERSONAL DATA
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              5. DISCLOSURES OF YOUR PERSONAL DATA
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              6. INTERNATIONAL TRANSFERS
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>7. DATA SECURITY</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>8. DATA RETENTION</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>9. YOUR LEGAL RIGHTS</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>10. GLOSSARY</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Purpose of this privacy notice</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              This privacy notice aims to give you information on how{" "}
            </span>
            <span style={{ fontSize: "medium" }}>
              <strong>cfsquad </strong>
            </span>
            <span style={{ fontSize: "medium" }}>
              collects and processes your personal data through your use of this
              website and application, including any data you may provide
              through its website and application when you sign up to our
              newsletter, our editorial programme or take part in a competition.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Its website and application are not intended for children and we
              do not knowingly collect data relating to children.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              It is important that you read this privacy notice together with
              any other privacy notice or fair processing notice we may provide
              on specific occasions when we are collecting or processing
              personal data about you so that you are fully aware of how and why
              we are using your data. This privacy notice supplements the other
              notices and is not intended to override them.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Controller</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>cfsquad</strong>
            </span>
            <span style={{ fontSize: "medium" }}>
              {" "}
              is the controller and responsible for your personal data
              (collectively referred to as "
            </span>
            <span style={{ fontSize: "medium" }}>
              <strong>cfsquad </strong>
            </span>
            <span style={{ fontSize: "medium" }}>
              ", "we", "us" or "our"; in this privacy notice).
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              You have the right to make a complaint at any time to the
              Information Commissioner's Office (ICO), the UK supervisory
              authority for data protection issues (www.ico.org.uk). We would,
              however, appreciate the chance to deal with your concerns before
              you approach the ICO so please contact us in the first instance.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>
                Changes to the privacy notice and your duty to inform us of
                changes
              </strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              This version was last updated on 12th July 2021 and historic
              versions can be obtained by contacting us.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              It is important that the personal data we hold about you is
              accurate and current. Please keep us informed if your personal
              data changes during your relationship with us.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Third-party links</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Our website and application may include links to third-party
              websites and applications, plug-ins and applications. Clicking on
              those links or enabling those connections may allow third parties
              to collect or share data about you. We do not control these
              third-party websites and applications and are not responsible for
              their privacy statements. When you leave our website and
              application, we encourage you to read the privacy notice of every
              website and application you visit.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              The data we collect about you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Personal data, or personal information, means any information
              about an individual from which that person can be identified. It
              does not include data where the identity has been removed
              (anonymous data).
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We may collect, use, store and transfer different kinds of
              personal data about you which we have grouped together follows:
            </span>
          </p>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Identity Data includes:
                </span>
              </p>
              <ul>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>Full name</span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      User name or similar identifier
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Account verification information
                    </span>
                  </p>
                </li>
              </ul>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Contact Data includes:
                </span>
              </p>
              <ul>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>Email addresses</span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>Telephone number</span>
                  </p>
                </li>
              </ul>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Technical Data includes:
                </span>
              </p>
              <ul>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>IP address</span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>Login data</span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Browser type and version
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Operating system, platform, and other technology on the
                      devices used to access this website and application
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Time zone setting and location
                    </span>
                  </p>
                </li>
              </ul>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Profile Data includes:
                </span>
              </p>
              <ul>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Your username and password
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>Title and name</span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Your place of work or study
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Your country of residence
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Links to your social media profiles
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Feedback and survey responses
                    </span>
                  </p>
                </li>
              </ul>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Usage Data includes information about how you use our website,
                  application, products and services.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0.92cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Marketing and Communications Data includes your preferences in
                  receiving marketing from us, third parties, and your
                  communication preferences.
                </span>
              </p>
            </li>
          </ul>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We also collect, use and share Aggregated Data such as statistical
              or demographic data for any purpose. Aggregated Data may be
              derived from your personal data but is not considered personal
              data in law as this data does not directly or indirectly reveal
              your identity. For example, we may aggregate your Usage Data to
              calculate the percentage of users accessing a specific website and
              application feature. However, if we combine or connect Aggregated
              Data with your personal data so that it can directly or indirectly
              identify you, we treat the combined data as personal data which
              will be used in accordance with this privacy notice.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We do not collect any Special Categories of Personal Data about
              you (this includes details about your race or ethnicity, religious
              or philosophical beliefs, sex life, sexual orientation, political
              opinions, trade union membership, information about your health
              and genetic and biometric data). Nor do we collect any information
              about criminal convictions and offences.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              If you fail to provide personal data where we need to collect
              personal data by law, or under the terms of a contract we have
              with you and you fail to provide that data when requested, we may
              not be able to perform the contract we have or are trying to enter
              into with you (for example, to provide you with goods or
              services). In this case, we may have to cancel a product or
              service you have with us but we will notify you if this is the
              case at the time.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              How is your personal data collected?
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We use different methods to collect data from and about you
              including through:
            </span>
          </p>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Direct interactions. You may give us your Identity, Contact,
                  and Profile Data by filling in forms or by corresponding with
                  us by post, phone, email or otherwise. This includes personal
                  data you provide when you:
                </span>
              </p>
              <ul>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      apply for our products or services
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      create an account on our website or application
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      subscribe to our service or publications
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      request marketing to be sent to you
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      enter a competition, promotion or survey
                    </span>
                  </p>
                </li>
                <li>
                  <p
                    style={{
                      marginBottom: "0cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      give us some feedback.
                    </span>
                  </p>
                </li>
              </ul>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Automated technologies or interactions. As you interact with
                  our website and application, we may automatically collect
                  Technical Data about your equipment, browsing actions and
                  patterns. We collect this personal data by using cookies,
                  server logs and other similar technologies. We may also
                  receive Technical Data about you if you visit other website
                  and applications employing our cookies.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Third parties or publicly available sources. We may receive
                  personal data about you from various third parties and public
                  sources as set out below:
                </span>
              </p>
              <ul>
                <li>
                  <p
                    style={{
                      marginBottom: "1.83cm",
                      lineHeight: "100%",
                      orphans: 2,
                      widows: 2,
                    }}
                    align="left"
                  >
                    <span style={{ fontSize: "medium" }}>
                      Technical Data from the following parties:
                    </span>
                  </p>
                </li>
              </ul>
            </li>
          </ul>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              1. Analytics providers, such as Google based inside and outside
              the EU
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              2. Advertising networks, such as Facebook based inside and outside
              the EU
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              3. Search information providers
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              border: "none",
              padding: "0cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            data-darkreader-inline-border-top
            data-darkreader-inline-border-right
            data-darkreader-inline-border-bottom
            data-darkreader-inline-border-left
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              4. Identity and Contact Data from data brokers or aggregators
              based inside and outside the EU.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              How we use your personal data
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We will only use your personal data when the law allows us to.
              Most commonly, we will use your personal data in the following
              circumstances:
            </span>
          </p>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Where we need to perform the contract we are about to enter
                  into or have entered into with you.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Where it is necessary for our legitimate interests (or those
                  of a third party) and your interests and fundamental rights do
                  not override those interests.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0.92cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Where we need to comply with a legal or regulatory obligation.
                </span>
              </p>
            </li>
          </ul>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              See below to find out more about the types of lawful basis that we
              will rely on to process your personal data.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Generally we do not rely on consent as a legal basis for
              processing your personal data other than in relation to sending
              third party direct marketing communications to you via email or
              text message.You have the right to withdraw consent to marketing
              at any time by contacting us.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Purposes for which we will use your personal data
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We have set out below a description of all the ways we plan to use
              your personal data, and which of the legal bases we rely on to do
              so. We have also identified what our legitimate interests are
              where appropriate.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Note that we may process your personal data for more than one
              lawful ground depending on the specific purpose for which we are
              using your data. Please contact us if you need details about the
              specific legal ground we are relying on to process your personal
              data where more than one ground has been set out in the table
              below.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Purpose/Activity Type of data Lawful basis for processing
              including basis of legitimate interest
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To register you as a new member or user and create a member
              profile for you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Identity</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(b) Contact</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Performance against Terms of Service and/or contract with you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Necessary for our legitimate interests (to develop our
              products/services and grow our business)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To register you as a new customer
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Identity</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Contact Performance of a contract with you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To process and deliver your order including:
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Manage payments, fees and charges
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Collect and recover money owed to us
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Identity</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(b) Contact</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(c) Financial</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(d) Transaction</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (e) Marketing and Communications
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Performance of a contract with you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Necessary for our legitimate interests (to recover debts due
              to us)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To manage our relationship with you which will include:
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Notifying you about changes to our terms or privacy policy
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Asking you to leave a review or take a survey
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Identity</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(b) Contact</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(c) Profile</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (d) Marketing and Communications
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Performance of a contract with you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Necessary to comply with a legal obligation
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (c) Necessary for our legitimate interests (to keep our records
              updated and to study how customers use our products/services)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To enable you to partake in a prize draw, competition or complete
              a survey
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Identity</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(b) Contact</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(c) Profile</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(d) Usage</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (e) Marketing and Communications
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Performance of a contract with you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Necessary for our legitimate interests (to study how customers
              use our products/services, to develop them and grow our business)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To administer and protect our business and its website and
              application (including troubleshooting, data analysis, testing,
              system maintenance, support, reporting and hosting of data)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Identity</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(b) Contact</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(c) Technical</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Necessary for our legitimate interests (for running our
              business, provision of administration and IT services, network
              security, to prevent fraud and in the context of a business
              reorganisation or group restructuring exercise)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Necessary to comply with a legal obligation
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To deliver relevant website and
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) Identity necessary for our legitimate interests
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) To application content and advertisements to you and measure
              or understand the effectiveness of the advertising we serve to you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(c) Contact</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(d) Profile</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(e) Usage</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (f) Marketing and Communications
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (g) Technical study how customers use our products/services, to
              develop them, to grow our business and to inform our marketing
              strategy
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To use data analytics to improve our website and application,
              products/services, marketing, customer relationships and
              experiences
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Technical</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) Usage Necessary for our legitimate interests (to define types
              of customers for our products and services, to keep our website
              and application updated and relevant, to develop our business and
              to inform our marketing strategy)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <br />{" "}
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To make suggestions and recommendations to you about goods or
              services that may be of interest to you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(a) Identity</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(b) Contact</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(c) Technical</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(d) Usage</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>(e) Profile</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Necessary for our legitimate interests (to develop our
              products/services and grow our business)
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Marketing</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We strive to provide you with choices regarding certain personal
              data uses, particularly around marketing and advertising. We have
              established the following personal data control mechanisms:
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Promotional offers from us</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We may use your Identity, Contact, Technical, Usage and Profile
              Data to form a view on what we think you may want or need, or what
              may be of interest to you. This is how we decide which products,
              services and offers may be relevant for you (we call this
              marketing).
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              You will receive marketing communications from us if you have
              requested information from us or purchased goods or services from
              us or if you provided us with your details when you entered a
              competition or registered for a promotion and, in each case, you
              have not opted out of receiving that marketing.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Third-party marketing</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We will get your express opt-in consent before we share your
              personal data with any company outside the{" "}
            </span>
            <span style={{ fontSize: "medium" }}>
              <strong>cfsquad</strong>
            </span>
            <span style={{ fontSize: "medium" }}> for marketing purposes.</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Opting out</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              You can ask us or third parties to stop sending you marketing
              messages at any time by logging into the website or application
              and checking or unchecking relevant boxes to adjust your marketing
              preferences or by following the opt-out links on any marketing
              message sent to you or by contacting us at any time.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Where you opt out of receiving these marketing messages, this will
              not apply to personal data provided to us as a result of a
              product/service purchase, warranty registration, product/service
              experience or other transactions.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Cookies</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              You can set your browser to refuse all or some browser cookies, or
              to alert you when websites set or access cookies. If you disable
              or refuse all cookies, please note that you will not be able to
              sign in to this website. For more information about the cookies we
              use, please see our cookie policy .
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Change of purpose</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We will only use your personal data for the purposes for which we
              collected it, unless we reasonably consider that we need to use it
              for another reason and that reason is compatible with the original
              purpose. If you wish to get an explanation as to how the
              processing for the new purpose is compatible with the original
              purpose, please contact us .
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              If we need to use your personal data for an unrelated purpose, we
              will notify you and we will explain the legal basis which allows
              us to do so.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Please note that we may process your personal data without your
              knowledge or consent, in compliance with the above rules, where
              this is required or permitted by law.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              <strong>Disclosures of your personal data</strong>
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We may have to share your personal data with the parties set out
              below for the purposes set out in the table in paragraph 4 above.
            </span>
          </p>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Internal Third Parties as set out in the Glossary .
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  External Third Parties as set out in the Glossary .
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0.92cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Third parties to whom we may choose to sell, transfer, or
                  merge parts of our business or our assets. Alternatively, we
                  may seek to acquire other businesses or merge with them. If a
                  change happens to our business, then the new owners may use
                  your personal data in the same way as set out in this privacy
                  notice.
                </span>
              </p>
            </li>
          </ul>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We require all third parties to respect the security of your
              personal data and to treat it in accordance with the law. We do
              not allow our third-party service providers to use your personal
              data for their own purposes and only permit them to process your
              personal data for specified purposes and in accordance with our
              instructions.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>International transfers</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We share your personal data within the{" "}
            </span>
            <span style={{ fontSize: "medium" }}>
              <strong>cfsquad </strong>
            </span>
            <span style={{ fontSize: "medium" }}>
              . This will involve transferring your data outside the European
              Economic Area (EEA).
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Many of our external third parties are based outside the European
              Economic Area (EEA) so their processing of your personal data will
              involve a transfer of data outside the EEA.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Whenever we transfer your personal data out of the EEA, we ensure
              a similar degree of protection is afforded to it by ensuring at
              least one of the following safeguards is implemented:
            </span>
          </p>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  We will only transfer your personal data to countries that
                  have been deemed to provide an adequate level of protection
                  for personal data by the European Commission. For further
                  details, see European Commission: Adequacy of the protection
                  of personal data in non-EU countries.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Where we use certain service providers, we may use specific
                  contracts approved by the European Commission which give
                  personal data the same protection it has in Europe. For
                  further details, see European Commission: Model contracts for
                  the transfer of personal data to third countries.{" "}
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0.92cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Where we use providers based in the US, we may transfer data
                  to them if they are part of the Privacy Shield which requires
                  them to provide similar protection to personal data shared
                  between the Europe and the US. For further details, see
                  European Commission: EU-US Privacy Shield.
                </span>
              </p>
            </li>
          </ul>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Please contact us if you want further information on the specific
              mechanism used by us when transferring your personal data out of
              the EEA.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>Data security</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We have put in place appropriate security measures to prevent your
              personal data from being accidentally lost, used or accessed in an
              unauthorised way, altered or disclosed. In addition, we limit
              access to your personal data to those employees, agents,
              contractors and other third parties who have a business need to
              know. They will only process your personal data on our
              instructions and
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              they are subject to a duty of confidentiality.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We have put in place procedures to deal with any suspected
              personal data breach and will notify you and any applicable
              regulator of a breach where we are legally required to do so.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>Data retention</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              How long will you use my personal data for?
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We will only retain your personal data for as long as necessary to
              fulfil the purposes we collected it for, including for the
              purposes of satisfying any legal, accounting, or reporting
              requirements.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              To determine the appropriate retention period for personal data,
              we consider the amount, nature, and sensitivity of the personal
              data, the potential risk of harm from unauthorised use or
              disclosure of your personal data, the purposes for which we
              process your personal data and whether we can achieve those
              purposes through other means, and the applicable legal
              requirements.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              By law we have to keep basic information about our customers
              (including Contact, Identity, Financial and Transaction Data) for
              six years after they cease being customers for tax purposes.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              In some circumstances you can ask us to delete your data: see
              [Request erasure] below for further information.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              In some circumstances we may anonymise your personal data (so that
              it can no longer be associated with you) for research or
              statistical purposes in which case we may use this information
              indefinitely without further notice to you.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>Your legal rights</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Under certain circumstances, you have rights under data protection
              laws in relation to your personal
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              data. Please click on the links below to find out more about these
              rights:
            </span>
          </p>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Request access to your personal data.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Request correction of your personal data.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Request erasure of your personal data.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Object to processing of your personal data.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Request restriction of processing your personal data.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Request transfer of your personal data.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0.92cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Right to withdraw consent.
                </span>
              </p>
            </li>
          </ul>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              If you wish to exercise any of the rights set out above, please
              contact us.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>No fee usually required</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              You will not have to pay a fee to access your personal data (or to
              exercise any of the other rights). However, we may charge a
              reasonable fee if your request is clearly unfounded, repetitive or
              excessive. Alternatively, we may refuse to comply with your
              request in these circumstances.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              What we may need from you
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We may need to request specific information from you to help us
              confirm your identity and ensure your right to access your
              personal data (or to exercise any of your other rights). This is a
              security measure to ensure that personal data is not disclosed to
              any person who has no right to receive it. We may also contact you
              to ask you for further information in relation to your request to
              speed up our response.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>Time limit to respond</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              We try to respond to all legitimate requests within one month.
              Occasionally it may take us longer than a month if your request is
              particularly complex or you have made a number of requests. In
              this case, we will notify you and keep you updated.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>Glossary</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>LAWFUL BASIS</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Legitimate Interest means the interest of our business in
              conducting and managing our business to enable us to give you the
              best service/product and the best and most secure experience. We
              make sure we consider and balance any potential impact on you
              (both positive and negative) and your rights before we process
              your personal data for our legitimate interests. We do not use
              your personal data for activities where our interests are
              overridden by the impact on you (unless we have your consent or
              are otherwise required or permitted to by law). You can obtain
              further information about how we assess our legitimate interests
              against any potential impact on you in respect of specific
              activities by contacting us
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Performance of Contract means processing your data where it is
              necessary for the performance of a contract to which you are a
              party or to take steps at your request before entering into such a
              contract.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Complying with a legal or regulatory obligation means processing
              your personal data where it is necessary for compliance with a
              legal or regulatory obligation that we are subject to.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>THIRD PARTIES</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>Internal Third Parties</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              Other companies in the C WZRD Limited Group (being at present WZRD
              Group Limited whose registered address is Unit 18 Poplar Drive
              Poplar Drive, Witton, Birmingham, United Kingdom, B6 7AD, Company
              Number: 12021691) acting as joint controllers or processors and
              who are based in the EU and provide IT and system administration
              services and undertake leadership reporting.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>External Third Parties</span>
          </p>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Service providers acting as processors based in and outside
                  the EU who provide IT, system administration and technology
                  services.
                </span>
              </p>
            </li>
            <li>
              <p
                style={{
                  marginBottom: "0cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  Professional advisers acting as processors or joint
                  controllers including lawyers, bankers, auditors and insurers
                  based in andoutside the EU who provide consultancy, banking,
                  legal, insurance and accounting services.
                </span>
              </p>
            </li>
          </ul>
          <ul>
            <li>
              <p
                style={{
                  marginBottom: "0.92cm",
                  lineHeight: "100%",
                  orphans: 2,
                  widows: 2,
                }}
                align="left"
              >
                <span style={{ fontSize: "medium" }}>
                  HM Revenue &amp;amp; Customs, regulators and other authorities
                  acting as processors or joint controllers based in the United
                  Kingdom who require reporting of processing activities in
                  certain circumstances.
                </span>
              </p>
            </li>
          </ul>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>YOUR LEGAL RIGHTS</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>You have the right to:</span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              1. Request access to your personal data (commonly known as a "data
              subject access request"). This enables you to receive a copy of
              the personal data we hold about you and to check that we are
              lawfully processing it.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              2. Request correction of the personal data that we hold about you.
              This enables you to have any incomplete or inaccurate data we hold
              about you corrected, though we may need to verify the accuracy of
              the new data you provide to us.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              3. Request erasure of your personal data. This enables you to ask
              us to delete or remove personal data where there is no good reason
              for us continuing to process it. You also have the right to ask us
              to delete or remove your personal data where you have successfully
              exercised your right to object to processing (see below), where we
              may have processed your information unlawfully or where we are
              required to erase your personal data to comply with local law.
              Note, however, that we may not always be able to comply with your
              request of erasure for specific legal reasons which will be
              notified to you, if applicable, at the time of your request.
            </span>
          </p>
          <p
            style={{
              marginBottom: "0.53cm",
              lineHeight: "100%",
              orphans: 2,
              widows: 2,
            }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              4. Object to processing of your personal data where we are relying
              on a legitimate interest (or those of a third party) and there is
              something about your particular situation which makes you want to
              object to processing on this ground as you feel it impacts on your
              fundamental rights and freedoms. You also have the right to object
              where we are processing your personal data for direct marketing
              purposes. In some cases, we may demonstrate that we have
              compelling legitimate grounds to process your information which
              override your rights and freedoms.
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              5. Request restriction of processing of your personal data. This
              enables you to ask us to suspend the processing of your personal
              data in the following scenarios:
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (a) if you want us to establish the data's accuracy
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (b) where our use of the data is unlawful but you do not want us
              to erase it
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (c) where you need us to hold the data even if we no longer
              require it as you need it to establish, exercise or defend legal
              claims
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              (d) you have objected to our use of your data but we need to
              verify whether we have overriding legitimate grounds to use it.
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            &nbsp;
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              6. Request the transfer of your personal data to you or to a third
              party. We will provide to you, or a third party you have chosen,
              your personal data in a structured, commonly used,
              machine-readable format. Note that this right only applies to
              automated information which you initially provided consent for us
              to use or where we used the information to perform a contract with
              you
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            &nbsp;
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            <span style={{ fontSize: "medium" }}>
              7. Withdraw consent at any time where we are relying on consent to
              process your personal data. However, this will not affect the
              lawfulness of any processing carried out before you withdraw your
              consent. If you withdraw your consent, we may not be able to
              provide certain products or services to you. We will advise you if
              this is the case at the time you withdraw your consent.
            </span>
          </p>
          <p
            style={{ marginBottom: "0cm", orphans: 2, widows: 2 }}
            align="left"
          >
            &nbsp;
          </p>
        </>
      </div>
      <Footer />
    </AnimationRevealPage>
  );
};
